...
simulated function Update() 
{
	...
	{ //If we do not have a valid action selected, ignore this update request
		...
		if (IsInsanityAbility(SelectedAbilityState.GetMyTemplateName()))
		{
			ShotDescription = GetInsanityEffectChancesString(SelectedAbilityState, Target) $ "\n" $ ShotDescription;
		}
...
static function bool IsInsanityAbility(name AbilityName)
{
    return AbilityName == 'Insanity' || AbilityName == 'VoidRiftInsanity';
}

/**
 * Returns formatted string with Insanity effect chances (Disorient / Panic / Mind Control)
 *
 * @param AbilityState   Ability being used (Insanity or variant)
 * @param TargetRef      Target unit reference
 *
 * @return string        Colored formatted chances string for ShotHUD
 */
function string GetInsanityEffectChancesString(
...
)
{
    ...
    local int MaxTier, MiddleTier, Idx;
    local array<float> TierValues;
    local float TierValue, LowTierValue, HighTierValue, TierValueSum;
    local float DisorientChance, PanicChance, MindControlChance;
    ...
    StatCalc = X2AbilityToHitCalc_StatCheck(AbilityState.GetMyTemplate().AbilityToHitCalc);
    ...
    // === Build tier probability distribution (same as RollForEffectTier) ===
    ...
    foreach Effects(Effect)
    {
        ...
        for (Idx = 1; Idx <= MaxTier; ++Idx)
        {
            if ((Effect.MinStatContestResult == 0 || Idx >= Effect.MinStatContestResult) &&
                (Effect.MaxStatContestResult == 0 || Idx <= Effect.MaxStatContestResult))
            {
                if (Effect.IsA('X2Effect_PersistentStatChange'))
                {
                    // Disorient
                    DisorientChance += TierValues[Idx - 1];
                }
                else if (Effect.IsA('X2Effect_Panicked'))
                {
                    PanicChance += TierValues[Idx - 1];
                }
                else if (Effect.IsA('X2Effect_MindControl'))
                {
                    MindControlChance += TierValues[Idx - 1];
                }
            }
        }
    }
    ...
    // Convert to %
    DisorientChance *= 100.0f;
    PanicChance *= 100.0f;
    MindControlChance *= 100.0f;

    // === Format output ===
    Result =
        class'UIUtilities_Text'.static.GetColoredText(
            "Disorient: " $ int(DisorientChance) $ "%",
            eUIState_Normal
        )
        $ " | " $
        class'UIUtilities_Text'.static.GetColoredText(
            "Panic: " $ int(PanicChance) $ "%",
            eUIState_Warning
        )
        $ " | " $
        class'UIUtilities_Text'.static.GetColoredText(
            "Mind Control: " $ int(MindControlChance) $ "%",
            eUIState_Bad
        );
...
function int GetHighestTierPossibleFromEffects(array<X2Effect> Effects)
{
